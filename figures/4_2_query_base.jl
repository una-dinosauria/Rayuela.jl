...
function make_plots_query_base(RPATH, dnames)
  ...
  for midx = 1:length( ms )
    ...
    for dataset_name = dnames
      ...
      for baseline = ["lsq", "src", "srd"]
        ...
        errorbar(total_times, recalls, yerr=stdevs, label="$lstart", fmt = "s", lw=linew)
      end
      ...
      for baseline = ["ervq", "rvq"]
        ...
        errorbar(total_times, recalls, yerr=stdevs, label="$(uppercase(baseline))", fmt = "d", lw=linew)
      end
      ...
      ax[:set_ylim](m == 7 ? [0.1, 0.5] : [0.2, 0.7])
  ...
  PyPlot.tight_layout()
  sfile = "/home/julieta/Desktop/recall_vs_time_$(dnames[1])_$(dnames[2])_100.pdf"
  savefig( sfile )
end
...
function make_plots_train_query_base(RPATH, dnames)
  ...
  running_times_sift_train = # encoding + codebook
    Dict("sr_64"       => 6.79 + 0.11, # fast codebook
         "sr_128"      => 20.35 + 0.96,
         "lsq_64"      => 6.79 + 4.76, # no fast codebook
         "lsq_128"     => 20.35 + 16.59,
         "sr_64_gpu"   => 0.85 + 0.11, # gpu, fast codebook
         "sr_128_gpu"  => 2.86 + 0.96,
         "lsq_64_gpu"  => 0.85 + 4.76, # gpu, no fast codebook
         "lsq_128_gpu" => 2.86 + 16.59,
         "chainq_sr_64"      => 11.40, # fast codebook
         "chainq_sr_128"     => 26.85, #
         "chainq_64"         => 12.46, # no fast codebook no gpu
         "chainq_128"        => 27.83, #
         "chainq_sr_64_gpu"  => 5.94, # gpu, fast codebook
         "chainq_sr_128_gpu" => 12.84, #
         "chainq_64_gpu"     => 6.88, # gpu, no fast codebook
         "chainq_128_gpu"    => 14.01, #
         "cq_64"       => 4.5*3600, # 4.5 hours on base; 42 min if trained on train
   ...
   running_times_sift_encode = # encoding + codebook
     Dict("sr_64"       => 7.59, # no GPU -- Seconds per iterations
          "sr_128"      => 25.27,
          "lsq_64"      => 7.59,
          "lsq_128"     => 25.27,
          "sr_64_gpu"   => 0.71, # GPU
  ...
  for midx = 1:length( ms )
    ...
    for dataset_name = dnames
      ...
      times_dict = running_times_sift_train
      base_dict = running_times_sift_encode
      ...
      # Load srd
      for baseline = ["lsq", "src", "srd"]
        # if (baseline == "src" || baseline == "srd") && m == 15
        #   plot([], [])
        #   continue
        # end
        recalls, stdevs, total_times = [], [], []
        for (niter, ilsiter) in zip(niters, ilsiters)
          meanrecall, stdrecall = load_recall_curves(ntrials, toplot, dataset_name, RPATH, "$(baseline)_m$(m)_it$niter.h5", ilsiter)
          println("$dataset_name $(baseline) m=$m"); print_recalls(meanrecall, stdrecall)
          ...
          push!(recalls, meanrecall[1])
          if baseline == "lsq"
            push!(total_times, init_time + (times_dict["lsq_$(8(m+1))"] * niter) + (base_dict["lsq_$(8(m+1))"] * ilsiter))
          else
            push!(total_times, init_time + (times_dict["sr_$(8(m+1))"] * niter) + (base_dict["sr_$(8(m+1))"] * ilsiter))
          end
        end
      ...
      for baseline = ["lsq", "src", "srd"]
        ...
        for (niter, ilsiter) in zip(niters, ilsiters)
          ...
          push!(recalls, meanrecall[1])
          if baseline == "lsq"
            push!(total_times, init_time + (times_dict["lsq_$(8(m+1))_gpu"] * niter) + (base_dict["lsq_$(8(m+1))_gpu"] * ilsiter))
          else
            push!(total_times, init_time + (times_dict["sr_$(8(m+1))_gpu"] * niter) + (base_dict["sr_$(8(m+1))_gpu"] * ilsiter))
          end
        end
      ...
      for baseline = ["ervq", "rvq"]
        ...
        for niter in niters
          ...
          init_time = baseline == "rvq" ? 0 : times_dict["rvq_$(8(m+1))"] * niter
          running_time = times_dict["$(baseline)_$(8(m+1))"] * niter
          encoding_time = base_dict["$(baseline)_$(8(m+1))"]
          push!(total_times, init_time + running_time + encoding_time)
        end
        # errorbar(total_times, recalls, yerr=stdevs, label="$(uppercase(baseline))", fmt = "-", lw=linew)
        errorbar(total_times, recalls, yerr=stdevs, label="$(uppercase(baseline))", fmt = "d", lw=linew)
      end
      ...
      for baseline = ["opq", "pq"]
        ...
        for niter in niters
          ...
          running_time = times_dict["$(baseline)_$(8(m+1))"] * niter
          encoding_time = base_dict["$(baseline)_$(8(m+1))"]
          push!(total_times, running_time + encoding_time)
        end
        # plot(total_times, recalls, label="$(uppercase(baseline)) $((m+1)*8) bits", m == 7 ? "--x" : "", lw=linew)
        errorbar(total_times, recalls, yerr=stdevs, label="$(uppercase(baseline))", fmt = "^", lw=linew)
      end
      ...
      # ymin = dataset_name == "mnist" ? 0.3 : ymin
      # ymin = dataset_name == "labelme" ? 0.2 : ymin
      ax[:set_ylim](m == 7 ? [0., 0.35] : [0.2, 0.6])
      # ax[:set_xlim]([1, dataset_name == "mnist" ? 10_000 : 1_000])
      ax[:set_xlim]([1, 100_000])
  ...
  PyPlot.tight_layout()
  PyPlot.tight_layout()
  sfile = "/home/julieta/Desktop/recall_vs_time_$(dnames[1])_$(dnames[2])_$(dnames[3])_100_128.pdf"
  savefig( sfile )
end

RPATH = "/home/julieta/.julia/v0.6/Rayuela/results/"
# make_plots_query_base(RPATH, ["mnist", "labelme"])
make_plots_train_query_base(RPATH, ["sift1m", "deep1m", "convnet1m"])
